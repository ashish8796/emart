import { ADD_PRODUCT_IN_SHOPPING_CART, REMOVE_PRODUCT_FROM_CART, SET_INTERNET_ERROR, SET_LIST_OF_PRODUCTS_IN_SHOPPING_CART, SET_SHOPPING_CART_ID } from "../actions/actionTypes";
...
const intialState: ShoppingCartState = {
  ...
  cartId: { id: localStorage.getItem('primary_cart_id') ? localStorage.getItem('primary_cart_id') : "", cartIdStatus: localStorage.getItem('primary_cart_id') ? 200 : false },
  productsList: [],
...
function shoppingCartReducer(state = intialState, action: MainAction) {
  switch (action.type) {
    ...
    case SET_INTERNET_ERROR: {
      return {
        ...state, cartProductStatus: action.payload, productsList: []
      }
    }
